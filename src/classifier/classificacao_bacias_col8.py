...
param = {    
    ...
    'assetROIs': {'id':'projects/mapbiomas-workspace/AMOSTRAS/col8/CAATINGA/ROIs/coletaROIsv6N2cluster'},
...
imagens_mosaic = ee.ImageCollection(param['asset_mosaic']).filter(
                            ee.Filter.eq('biome', param['bioma'])).filter(
                                ee.Filter.inList('satellite', lstSat)).select(arqParams.featuresreduce)
