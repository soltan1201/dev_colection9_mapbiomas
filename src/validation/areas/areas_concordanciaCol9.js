...
var class_col90 = ee.ImageCollection(param.asset_integration)
                        .filter(ee.Filter.eq('version','0-10'))
                        .mosaic().updateMask(rasterCaat)
...
years.forEach(function(year_j) {
    ...
    classes.forEach(function(class_i) {
        ...
        var conc = ee.Image(0).where(col8_j.eq(class_i).and(col9_j.eq(class_i)), 1) // [1]: Concordância entre col8 e 9
                            .where(col8_j.neq(class_i).and(col9_j.eq(class_i)), 2)  // [3]: discordância que vem da 8 
                            .where(col8_j.eq(class_i).and(col9_j.neq(class_i)), 3)  // [2]: discordância que vem da 9
