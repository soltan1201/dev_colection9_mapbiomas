...
def buildingPlots_x_Class(df_class, lstclass, lstVersion):
    coluna = 3
    ...
    fig.update_layout(height=800, width=1600, title_text="Bioma Caatinga " , showlegend=False)
    fig.update_layout(showlegend=True)
    fig.show()
    fig.write_image("plotBioma_CaatingaV2.png")
...
for bacia_sel in nameBacias[:]:    
    ...
    for nfile in filesAreaCSV:        
        if '_' + bacia_sel + '_' in nfile :  #  and 'Col7.1' in nfile
            ...
            # print(df_tmp.head(4))
            df_tmp['area'] = df_tmp['area'].apply(pd.to_numeric, downcast='float', errors='coerce')# print(df_tmp.head(4))
            df_tmp['area'] = df_tmp['area'].round(2)
            df_tmp = df_tmp.drop(['system:index', '.geo'], axis='columns')
            # print(df_tmp.head(4))            
            lst_df.append(df_tmp)

print(f"Foram carregadas {len(lst_df)} tabelas ")
concat_df  = pd.concat(lst_df, axis=0, ignore_index=True)
# concat_df = df_tmp
print("temos {} filas ".format(concat_df.shape))
print(concat_df.head(20))
print(" ")
# col1 = ['offer_id', 'affiliate_id', 'affiliate_source', 'advertiser_id', 'affiliate_sub5']
# df1.groupby(col1)[["payout_cents", "revenue_cents"]].sum()
columnaSel = ['classe', 'year', 'version']
newDF = concat_df.groupby(columnaSel, as_index = False)[['area']].sum()
# Example 4: Use the to_frame method
newDF = newDF.reset_index()
print(newDF.head(20))
lst_vers = [kk for kk in newDF.version.unique()]
print("versoes = ", lst_vers)
buildingPlots_x_Class(newDF, classes, lst_vers)
